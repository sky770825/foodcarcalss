...
async function submitToGoogleSheets(formData) {
  ...
  try {
    ...
    if (formData.action === 'updatePayment') {
      dataToSend.action = 'updatePayment';
      dataToSend.rowNumber = formData.rowNumber;
      dataToSend.payment = formData.payment;
      console.log('更新付款狀態操作，行號:', formData.rowNumber, '新狀態:', formData.payment);
    }
...
function mergeSheetsDataToCalendar() {
  ...
  if (!sheetsBookings || sheetsBookings.length === 0) {
    console.log('⚠️ 沒有Google Sheets數據');
    console.log('✅ 日曆將顯示為空');
    renderCalendar();
    return;
  }
  ...
  sheetsBookings.forEach((booking, index) => {
    ...
    const newEvent = {
      ...
      rowNumber: booking.rowNumber || null, // ✅ 保存行號，用於後續更新
      originalLocation: originalLocation // 保存原始場地名稱
    };
    
    // ✅ 診斷：檢查 rowNumber 是否存在
    if (index < 3) { // 只顯示前3筆
      console.log(`事件 #${index + 1}: ${booking.vendor}`);
      console.log(`  - rowNumber from Sheets: ${booking.rowNumber}`);
      console.log(`  - rowNumber in newEvent: ${newEvent.rowNumber}`);
    }
...
document.addEventListener('DOMContentLoaded', async function() {
  ...
  // 初次載入時同步Google Sheets數據（v3.2.3：快取優先，完整載入後顯示）
  if (GOOGLE_SHEETS_CONFIG.enabled) {
    // 記錄開始時間
